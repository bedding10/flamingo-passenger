...
const assetFiles=readdirSync(join(root,"assets"));check(!assetFiles.some((f)=>/^vehicle-.*\.(png|jpg|jpeg)$/i.test(f)),"vehicle artwork uses managed assets");check(assetFiles.filter((f)=>/^vehicle-.*\.webp$/i.test(f)).every((f)=>readFileSync(join(root,"assets",f)).length<=10000),"offline vehicle fallbacks stay tiny");
