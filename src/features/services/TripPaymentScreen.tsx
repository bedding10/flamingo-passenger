...
export function TripPaymentScreen({ navigation, route }: NativeStackScreenProps<RootStackParamList, "TripPayment">) {
  ...
  const checkoutKeys = useRef(new Map<PassengerPaymentMethod["method"], string>());
  const checkout = useMutation({
    mutationFn: (method: PassengerPaymentMethod["method"]) => {
      const key = checkoutKeys.current.get(method) ?? createIdempotencyKey(`checkout:${route.params.tripId}:${method}`);
      checkoutKeys.current.set(method, key);
      return passengerServicesApi.checkoutTrip(route.params.tripId, method, key);
    },
    onSuccess: async (result, method) => {
      checkoutKeys.current.delete(method);
      await Promise.all([
        client.invalidateQueries({ queryKey: ["trip-payment", route.params.tripId] }),
        client.invalidateQueries({ queryKey: ["wallet"] }),
      ]);
      if (result.checkout.checkoutUrl) await Linking.openURL(result.checkout.checkoutUrl);
    },
  });
