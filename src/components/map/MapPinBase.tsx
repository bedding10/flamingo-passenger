...
export const PIN_GEOMETRY = {
  headSize: 34,
  /** Confirmed points are a bare disc: no stem, no dot, so smaller reads better. */
  headSizeCompact: 26,
  headBorderWidth: 2,
  stemHeight: 16,
  stemWidth: 2,
...
export type MapPinBaseProps = {
  ...
  hideBubble?: boolean
  /**
   * "full": head + stem + origin dot. Used while the user is dragging the map
   * to place a point by hand.
   * "compact": the head disc alone - no stem, no dot underneath. Used for the
   * permanent markers of an already confirmed pickup / drop-off.
   */
...
const MapPinBase: React.FC<MapPinBaseProps> = ({
  ...
  const headSize = compact
    ? PIN_GEOMETRY.headSizeCompact
    : PIN_GEOMETRY.headSize
  const glyphScale = compact ? headSize / PIN_GEOMETRY.headSize : 1
  ...
  const head = useMemo(
    () => (
      <View
        style={[
          styles.headInner,
          glyphScale !== 1 ? { transform: [{ scale: glyphScale }] } : null,
        ]}
        pointerEvents="none"
      >
        {glyph}
  ...
  return (
    <View style={styles.root} pointerEvents="none" testID={testID}>
      ...
      <Animated.View style={bodyStyle}>
        <View style={[styles.head, { width: headSize, height: headSize }]}>
          <Svg
            width={headSize}
            height={headSize}
          ...
          >
            <Circle
              cx={headSize / 2}
              cy={headSize / 2}
              r={headSize / 2 - PIN_GEOMETRY.headBorderWidth / 2}
              fill={headFill}
              stroke={headStroke}
        ...
        {/* Short tapered stem. A CONFIRMED point has no stem at all: it is
            a bare disc planted on the map. */}
        {compact ? null : (
          <Svg
      ...
      {/* Ground zone: contact shadow + the origin dot (never moves). Both
          belong to the manual-placement choreography only. */}
      {compact ? null : (
